...
export default function Home() {
  ...
  const carouselItems = [
    { title: '교육과정 설명회', caption: '새로운 교육과정에 대한 설명회', img: '/carousel/carousel-1.png' },
    { title: '새내기 새로배움터', caption: '새내기들을 위한 행사', img: '/carousel/carousel-2.png' },
    { title: '레플리카 공동구매', caption: '정보대학 레플리카 제작ㆍ공구', img: '/carousel/carousel-3.png' },
    { title: '동아리 박람회', caption: '정보대학 동아리를 소개하는 행사', img: '/carousel/carousel-4.png' },
    { title: 'NE:XT Contest', caption: '정보대학 소프트웨어 경진대회', img: '/carousel/carousel-5.png' },
    { title: 'Global Mentoring', caption: '국제학우 멘토ㆍ멘티', img: '/carousel/carousel-6.png' },
    { title: 'E-Sports 대회', caption: '친목을 도모하는 테트리스 대회', img: '/carousel/carousel-7.png' },
    { title: '연사 초청 강연', caption: '학생들에게 조언을 주는 강연', img: '/carousel/carousel-8.png' },
    { title: '스승의 날 이벤트', caption: '교수님들께 감사의 메시지 전달', img: '/carousel/carousel-9.png' },
    { title: '오운완 챌린지', caption: '정보대 학우들의 건강 챙기기', img: '/carousel/carousel-10.png' },
    { title: '정보대, 낭만있을지도', caption: '정보대 학우들의 낭만 있는 여행 모음', img: '/carousel/carousel-11.png' },
    { title: '백준 챌린지', caption: '정보대 학우들의 코딩 능력 향상', img: '/carousel/carousel-12.png' },
    { title: '슬로건 공모전', caption: '정보대학의 정체성', img: '/carousel/carousel-13.png' },
    { title: 'Uni-Con', caption: '경영ㆍ공대ㆍ디조와 협업한 창업공모전', img: '/carousel/carousel-14.png' },
    { title: '정보인의 날', caption: '정보대학 축제', img: '/carousel/carousel-15.png' },
    { title: '일일호프', caption: '간호대학과 협업한 행사', img: '/carousel/carousel-16.png' },
  ];
  ...
  // ========= Clubs carousel data =========
  const clubsItems = [
    { name: 'AIKU', desc: '학술 | AI / 딥러닝 학회', logo: '/clubs/club-aiku.svg' },
    { name: 'ALPS', desc: '학술 | 알고리즘 / PS', logo: '/clubs/club-alps.svg' },
    { name: 'CAT&DOG', desc: '학술 | 게임 개발', logo: '/clubs/club-catdog.svg' },
    { name: 'GDGoC KU', desc: '학술 | 개발 커뮤니티', logo: '/clubs/club-gdsc.png' },
    { name: 'KUICS', desc: '학술 | 보안 / 해킹', logo: '/clubs/club-kuics.svg' },
    { name: 'KWEB', desc: '학술 | 웹/앱 개발 학회', logo: '/clubs/club-kweb.svg' },
    { name: 'LIBERTY', desc: '예체능 | 밴드', logo: '/clubs/club-liberty.svg' },
    { name: '소아달', desc: '예체능 | 사물놀이', logo: '/clubs/club-soadal.svg' },
  ];
  ...
  function getClubsPageWidth() { return clubsPageWidthRef.current; }
  useEffect(() => {
      ...
      <main ref={containerRef} className={styles.canvas} tabIndex={0}>
        ...
        <section className={styles.snapSection} aria-label="Hero">
          ...
          >
            <svg width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M15 25 L30 35 L45 25" stroke="#FBFBFB" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>
        </section>
        ...
        <section className={styles.snapSection} aria-label="About">
          ...
          <div className={styles.aboutSymbol} aria-hidden="true">
            <Image src="/about-vectors.svg" alt="" width={239} height={232} priority />
          </div>
          <div className={styles.aboutWrap}>
            ...
            <p className={styles.aboutTitle}>‘서로의 꿈을 이어 만드는 무한한 미래’</p>
            <p className={styles.aboutBody}>
              이음의 슬로건입니다.
              <br />
              저희 이음은 신입생 여러분들을 정보대학과 하나로 이어 새롭게 만들어갈 무한한 미래를 기대하고 있습니다.
            </p>
            <button className={styles.aboutButton} type="button">LEARN MORE</button>
          </div>
        </section>

        {/* Section 2.5: Carousel - 이음의 1년 */}
        <section className={`${styles.snapSection} ${styles.carouselSection}`} aria-label="이음의 1년">
          ...
          <div className={styles.carouselRow}>
            <button className={styles.weekNavigationButton} aria-label="이전" onClick={() => {
              const track = document.querySelector(`.${styles.carouselTrack}`);
              if (!track) return;
              const pageWidth = getPageWidth();
              const pagesLength = Math.ceil(carouselItems.length / itemsPerPage);
                ...
                {extendedCarouselItems.map((item, i) => (
                  <div className={styles.carouselCard} key={i}>
                    <div className={styles.carouselImageBox}>
                      <Image src={item.img} alt="" width={250} height={313} className={styles.carouselImage} />
                    </div>
                    <div className={styles.carouselCardTitle}>{item.title}</div>
                    <div className={styles.carouselCardCaption}>{item.caption}</div>
                  </div>
                ))}
              </div>
            </div>

            <button className={styles.weekNavigationButton} aria-label="다음" onClick={() => {
              const track = document.querySelector(`.${styles.carouselTrack}`);
              if (!track) return;
              const pageWidth = getPageWidth();
              const pagesLength = Math.ceil(carouselItems.length / itemsPerPage);
        ...
        <section className={`${styles.snapSection} ${styles.clubsSection}`} aria-label="정보대학 동아리">
          <div className={styles.clubsHeader}>
            <h2 className={styles.clubsTitle}>정보대학 동아리</h2>
            <p className={styles.clubsSubtitle}>정보대학에는 다양한 분야의 학생 동아리들이 있습니다.<br />정보대학 동아리연합회에 등록된 8개 동아리를 소개해드립니다.</p>
          </div>
          ...
          <div className={styles.clubsRow}>
            <button className={styles.weekNavigationButton} aria-label="이전" onClick={() => {
              ...
              const pagesLength = Math.ceil(clubsItems.length / clubsItemsPerPage);
              const current = Number(track.getAttribute('data-index') || '1');
              const next = current - 1;
              track.style.transition = 'transform 0.4s ease';
                  ...
                  <div className={styles.clubCard} key={i}>
                    <GlassContainer className={styles.clubGlass} radius={50}>
                      <div className={styles.logoBox}>
                        <Image src={c.logo} alt="" width={180} height={140} className={styles.logoImg} />
                      </div>
                    </GlassContainer>
        ...
        <section className={`${styles.snapSection} ${styles.snapSectionLast}`} aria-label="Visit">
